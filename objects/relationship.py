...
def make_disarm_subtechnique_relationship(source, target, marking_id, identity_id, date, description, external_references):

    relationship = Relationship(
       id ="relationship--{}".format(uuid.uuid5(namespace=NAMESPACE, name="{}+{}".format(source,target))
...
def make_disarm_subtechnique_relationships(techniques, identity_id, marking_id, date):
    ...
    for technique in techniques:
        technique_ids[technique["external_references"][0]["external_id"]] = (technique["id"], technique["external_references"][0])

    relationships = []
    for technique in techniques:
        if technique["x_mitre_is_subtechnique"]:
            source_ext_ref = technique["external_references"][0]
            technique_id, target_ext_ref = technique_ids[source_ext_ref["external_id"].split(".")[0]]
